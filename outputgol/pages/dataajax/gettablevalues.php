...
if ($_REQUEST['types'] == 'objecttable') {
    $aColumns = array('id', 'objectname','objectname_tamil');
    $sIndexColumn = "id";
    $editpage = ($_REQUEST['db_table_for'] == 'live') ? "editbill" : "editbill";
    $sTable = "object";
}
...
if ($_REQUEST['types'] == 'monthlyreturntable') {
    $aColumns = array('id', 'date','currentdate', 'customeridname','loanid', 'status');
    $sIndexColumn = "id";
    // $editpage = ($_REQUEST['db_table_for'] == 'live') ? "editbill" : "editbill";
    $sTable = "`monthly_interest`";
}

if ($_REQUEST['types'] == 'partialreturntable') {
    $aColumns = array('id', 'date','currentdate', 'customeridname','loanid', 'status');
    $sIndexColumn = "id";
    // $editpage = ($_REQUEST['db_table_for'] == 'live') ? "editbill" : "editbill";
    $sTable = "`partial_return`";
}

if ($_REQUEST['types'] == 'fullyreturntable') {
    $aColumns = array('id', 'date','currentdate', 'customeridname','loanid', 'status');
    $sIndexColumn = "id";
    // $editpage = ($_REQUEST['db_table_for'] == 'live') ? "editbill" : "editbill";
    $sTable = "`monthly_return`";
}


if ($_REQUEST['types'] == 'returntable') {
    $aColumns = array('id', 'date','currentdate', 'customeridname','loadid', 'status');
    $sIndexColumn = "id";
    // $editpage = ($_REQUEST['db_table_for'] == 'live') ? "editbill" : "editbill";
    $sTable = "`monthly_interest`";
}
...
while ($aRow = $rResult->fetch(PDO::FETCH_ASSOC)) {
    ...
    for ($i = 0; $i < count($aColumns1); $i++) {
        ...
        } else if ($_REQUEST['types'] == 'monthlyreturntable') {
            if ($aColumns1[$i] == $sIndexColumn) {
                $row[] = $k;
            } elseif ($aColumns1[$i] == 'loadid') {
    ...
    } elseif (($_REQUEST['types'] != 'fullyreturntable') && ($_REQUEST['types'] != 'partialreturntable') && ($_REQUEST['types'] != 'monthlyreturntable') && ($_REQUEST['types'] != 'objecttable') && (($_REQUEST['types'] != 'financecustomertbale') && ($_REQUEST['types'] != 'returntable') && ($_REQUEST['types'] != 'billtable') && ($_REQUEST['types'] != 'bankstatustable') && ($_REQUEST['types'] != 'suppliertable')  && ($_REQUEST['types'] != 'purchasetable') && ($_REQUEST['types'] != 'salestable') && ($_REQUEST['types'] != 'silverobjecttable'))) {
