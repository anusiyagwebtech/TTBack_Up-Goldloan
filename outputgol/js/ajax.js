 ...
 function deleteimage1(a, b, c, d, e, f)
{
    ...
    if (a != '' && b != '' && c != '' && d != '' && e != '' && f != '')
    {
        document.getElementById("delimage1").innerHTML = '<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>';
        oRequestsubcat.open("POST", Settings.base_url + "config/functions_ajax.php", true);
        oRequestsubcat.onreadystatechange = delimg11;
        oRequestsubcat.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        oRequestsubcat.send("image=" + a + "&id=" + b + "&table=" + c + "&path=" + d + "&images=" + e + "&pid=" + f);
        //console.log(a, b, c, d, e, f);
    }
...
function delimg11()
{
    if (oRequestsubcat.readyState == 4)
    {
        if (oRequestsubcat.status == 200)
        {
            document.getElementById("delimage1").innerHTML = oRequestsubcat.responseText;
        } else
        {
            document.getElementById("delimage1").innerHTML = oRequestsubcat.responseText;
        }
