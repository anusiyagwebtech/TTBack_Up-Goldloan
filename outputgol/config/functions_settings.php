...
function addgeneral($gold_rate,$silver_rate,$gid) {
    global $db;

    $resa = $db->prepare("UPDATE `generalsettings` SET `gold_rate`=?,`silver_rate`=? WHERE `generalid`=?");
    $resa->execute(array(trim($gold_rate),trim($silver_rate), $gid));
